...
class Waldbrandsimulation:
    ...
    def ErstelleFeld(self, rnd):
        ...
        if rnd == True:
            feld = [
                np.array(
                    [
                        [randint(0, randint(0, 1)) * 4 for j in range(self.size)]
                        for i in range(self.size)
                    ]
                ),
                np.array(
                    [[[10, 0, 10] for j in range(self.size)] for i in range(self.size)]
                ),
            ]  # status, brennpotential, time, feuchte
        else:
            feld = [
                np.array([[0 for j in range(self.size)] for i in range(self.size)]),
                np.array(
                    [[[10, 0, 10] for j in range(self.size)] for i in range(self.size)]
                ),
            ]  # status, brennpotential, time, feuchte
    ...
    def BearbeiteZelle(self, f, i, j, wr, ws, rs, rmoisture):
        ...
        if (
            (f[0][i][j] == 0)
            and (self.SummeBrennstärke(f, i, j, wr, ws) - rmoisture)
            >= int(f[1][i][j][2])
            and float(feld[1][i][j][0]) > 0
        ):
            status = 1  # /
...
while brennende_fläche > 0:
    ...
    for i in range(size):
        for j in range(size):
            ...
            if generation >= regenbeginn:
                ...
                feld2[1][i][j], feld2[0][i][j], change = simulation.BearbeiteZelle(
                    feld, i, j, windrichtung, windstärke, regenstärke, regenfeuchte
                )

            else:
                feld2[1][i][j], feld2[0][i][j], change = simulation.BearbeiteZelle(
                    feld, i, j, windrichtung, windstärke, 0, 0
                )
    ...
    np.save(
        simulation.path("data\\gen" + str(generation)), np.array(changes)
    )  # speichere Array
    changes = []
...
if simulation.Argumente("nogui") == True:
    os.system(simulation.command("render.py intervall=" + intervall))
else:
    os.system(simulation.command("render_gui.py intervall=" + intervall))
